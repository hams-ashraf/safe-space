...
export default function SymptomsChecking() {
  ...
  // تحديث طريقة حساب الخطوات بناءً على عدد الأسئلة اللي هتيجي من الباك إند
  ...
  const handleStartQuestions = async () => {
    ...
    try {
      ...
      // التعديل هنا: rawData دلوقتي عبارة عن Array من الـ Objects
      // كل Object فيه (question) و (options)
      // مفيش داعي للـ map اللي بيحط Yes/No
...
const handleFinalSubmit = async () => {
  setLoading(true);
  try {
    // 1. تجميع الإجابات في مصفوفة نصوص واضحة جداً
    const analysisArray = questions.map((q, index) => {
      const answer = answers[index] || "No answer";
      // بنبعت السؤال وجنبه الإجابة اللي المستخدم اختارها كنص
      return `Question: ${q.question} - User Answer: ${answer}`;
    });

    // 2. إرسال البيانات للـ API
    const response = await analyzeSymptomsApi({
      mood: selectedFeelings, // دي مصفوفة زي ["Sad", "Anxious"]
      analyze: analysisArray,  // دي المصفوفة اللي الـ AI هيحللها
    });

    let finalResult = response.data;
    
    // تأكدي إن الداتا اللي راجعة بتتحول لـ JSON لو مبعوتة كـ String
    if (typeof finalResult === "string") {
      finalResult = JSON.parse(finalResult);
    }
        ...
        {showFeelings ? (
          ...
          <div className="flex-grow-1">
            {questions.length > 0 ? (
              <>
                <h3 className="mb-4 fw-semibold">{questions[currentQuestion]?.question}</h3>
                {/* التعديل هنا: بنعرض الـ options اللي جاية من الباك إند لكل سؤال */}
                {questions[currentQuestion]?.options?.map((option, index) => (
                  <label key={index} className={`option-label ${answers[currentQuestion] === option ? "selected" : ""}`}>
                    <span className="radio-check-circle">{answers[currentQuestion] === option ? "✓" : ""}</span>
                    <span className="fw-medium">{option}</span>
